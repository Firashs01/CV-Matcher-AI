...
var parseMustache = (template) => {
  configureMustache();
  return mustacheTemplateToNodes(mustache_default.parse(template));
};
  ...
  async formatMessages(values) {
    ...
    for (const promptMessage of this.promptMessages) if (promptMessage instanceof BaseMessage) resultMessages.push(await this._parseImagePrompts(promptMessage, allValues));
      ...
      if (this.templateFormat === "mustache") inputValues = { ...allValues };
        ...
        if (!(inputVariable in allValues) && !(isMessagesPlaceholder(promptMessage) && promptMessage.optional)) throw addLangChainErrorFields(new Error(`Missing value for input variable \`${inputVariable.toString()}\``), "INVALID_PROMPT_INPUT");
...
var prompts_exports = __exportAll({
